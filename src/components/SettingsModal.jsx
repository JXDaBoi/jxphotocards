...
export default function SettingsModal({ onClose, standalone }) {
  ...
  const content = (
    <div className={standalone ? "" : "modal-content glass-panel"} style={standalone ? { maxWidth: '600px' } : { maxWidth: '450px', maxHeight: '90vh', overflowY: 'auto' }}>
      ...
      <div className="modal-actions" style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-start', marginTop: '2.5rem' }}>
        <button type="button" onClick={handleSave} className="btn-primary" disabled={saving}>{saving ? 'Syncing...' : 'Save Globally'}</button>
        {!standalone && <button type="button" onClick={onClose} className="btn-secondary" disabled={saving}>Cancel</button>}
      </div>
    </div>
  );
