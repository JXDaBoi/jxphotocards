...
export default function GalleryGrid({ 
  ...
  const filtered = photocards.filter(card => {
    ...
    const matchStatus = !statusFilter || statusFilter === 'All' || (card.status || 'Owned') === statusFilter;
    const matchGroup = !groupFilter || groupFilter === 'All' || card.group === groupFilter;
  ...
  return (
    <div className="gallery-container">
      {!globalSettings?.showcaseMode && (
        <div className="admin-toolbar glass-panel" style={{ padding: '1rem', marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
            <span style={{ fontWeight: 600 }}>Grid Mode:</span>
          ...
          {isAdminView && (
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              ...
              <button 
                ...
                style={batchMode ? { background: 'var(--accent)', color: 'white', borderColor: 'var(--accent)' } : {}}
              >
                {batchMode ? 'Cancel Setup' : '⚡ Batch Edit'}
              </button>
            </div>
          )}
        </div>
      ...
      {batchMode && selectedCards.size > 0 && (
         <div className="glass-panel slide-in" style={{ padding: '1rem', marginBottom: '1.5rem', display: 'flex', gap: '1rem', alignItems: 'center', background: 'var(--accent)', color: 'white' }}>
            <span>⚡ {selectedCards.size} Selected</span>
            <button className="btn-secondary" style={{ padding: '0.4rem 0.8rem', background: 'white', color: 'var(--accent)', border: 'none' }} onClick={() => handleBatchStatus('Owned')}>Mark Owned</button>
            <button className="btn-secondary" style={{ padding: '0.4rem 0.8rem', background: 'white', color: 'var(--accent)', border: 'none' }} onClick={() => handleBatchStatus('Wishlist')}>Mark Wishlist</button>
         </div>
      )}
          ...
          {filtered.map(card => {
            ...
            return (
              <div key={card.id} style={{ position: 'relative' }} onClick={() => batchMode ? toggleSelectCard(card.id) : null}>
                 {batchMode && (
                   <div style={{ position: 'absolute', top: '10px', left: '10px', width: '25px', height: '25px', borderRadius: '50%', background: isSelected ? 'var(--accent)' : 'white', zIndex: 20, border: '2px solid rgba(0,0,0,0.2)', transition: 'all 0.2s', transform: isSelected ? 'scale(1.1)' : 'scale(1)' }}>
                     {isSelected && <span style={{ color: 'white', display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>✓</span>}
                   </div>
                 )}
                <PhotocardItem 
                  card={card} 
                  onDelete={isAdminView && !batchMode ? () => deletePhotocard(card.id) : null} 
                  onEdit={isAdminView && !batchMode ? () => setEditingCard(card) : null}
